            ...
            if f.is_valid():
                ...
                try:
                    ...
                    for r in self.recipients.filter(active=True):
                        ...
                        subject = engines['django'].from_string(u'{%% autoescape off %%}%s{%% endautoescape %%}' % self.email_subject).render(ctx)
                        message = engines['django'].from_string(u'{%% autoescape off %%}%s{%% endautoescape %%}' % self.email_text).render(ctx)
